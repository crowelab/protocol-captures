...
def args():
	...
	seqlogo_parser.add_argument("--path","-lp", dest="logo_path", default="/dors/meilerlab/apps/scripts/sequence_analysis/weblogo-3.3/weblogo",help="What is the path to weblogo software? Defaults to meilerlab enviroment")
	...
	seqlogo_parser.add_argument("--stack_width", dest="s_stack_width", default="10.8",help="How wide is each stack in the logo. Value of 25 is useful for x-axis labels >3 characters and 30 for labels as 'sequence_numbers'.")
	...
	seqlogo_parser.add_argument("--fine_print", dest="s_fine",default="deep_analysis", help="Fine Print")
	...
	seqlogo_parser.add_argument("--labels", dest="s_labels", choices=['sequence','numbers',"sequence_numbers"], default="sequence", help="The x-axis labels can either take on the native residues sequence given with a native pdb file or the numbering of the pdb residue.")
	...
	# check to see that there is one positional argument
	...
	if 'd' in arg.o_files and 'nd' in arg.o_files:
		parser.exit(status=1, message="\nRunning the design and native design analysis simultaneously results in suspect output tables.\nThis is a bug that should be fixed but has not been. I think it results because the input dictionary the first time through is directly edited and then becomes the input dictionary the second time through.\nRun the script twice instead, once for each output table.")
	...
	if arg.logo and ('d' not in arg.o_files and 'nd' not in arg.o_files):
		 parser.exit("You have asked for sequence logos but not specified design or native design output (eg. -s d or -s nd)")
	elif arg.logo and ('d' in arg.o_files or 'nd' in arg.o_files):
		arg.labels='numbers'
